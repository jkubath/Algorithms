...
public class Main {
	...
	public static void main(String[] args) {
		/* Initialization */
		//int[] arrayLengths = {5, 10, 50, 100, 1000};
		...
		for(int index = 0; index < arrayLengths.length; index++) {
			...
			for( int runCount = 0; runCount < totalRuns; runCount++) {
				...
				//call the algorithm
				//mult_timed(a, b, c, a.length);
				count = mult(a, b, c, a.length);
				//count = mult_simplified(a, b, c, a.length);
	...
	public static void mult_timed(int[][] a, int[][] b, int[][] c, int n) {
		for(int i = 0; i < n; i++) {
			for(int j = 0; j < n; j++) {
				c[i][j] = 0;
				for(int k = 0; k < n; k++) {
	...
	public static int mult(int[][] a, int[][] b, int[][] c, int n) {
		int count = 0;
	...
	public static int mult_simplified(int[][] a, int[][] b, int[][] c, int n) {
		int count = 0;
