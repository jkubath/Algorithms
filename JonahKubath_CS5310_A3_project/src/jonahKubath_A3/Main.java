...
public class Main {
		...
		for(int z = 0; z < fileCount; z++) {
			...
			for(int j = 0; j < runs; j++) {
				...
				if(fileChoice == 0 || fileChoice == 1)
					buildMessage(tree, list2);
			}
			//Print average times
			System.out.println("Build heap - " + (time[0] / runs) / 1000 + " ms");
	...
	public static void buildMessage(Node tree, Node list) {
		Node index = list;
		...
		while(index != null) {
			...
			System.out.print("\tWeighted Depth  - " + message[1] + "\n");
			System.out.print("\tWeighted Length - " + message[0].length() + "\n");
			//System.out.println();
			index = index.getLeft();
		}
	}
